...
public class CommonHelper {
	...
	public static String checkParam(Object reqMain) {
		if (validator == null) {
			ValidatorFactory factory = Validation.buildDefaultValidatorFactory();
			validator = factory.getValidator();
		}
		Set<ConstraintViolation<Object>> constraintViolations = validator.validate(reqMain);
		if (constraintViolations.size() > 0) {
			String errrMsg = "";
			for (ConstraintViolation<Object> constraintViolation : constraintViolations) {
				errrMsg += constraintViolation.getMessage() + ",";
			}
			return "参数校验不通过: "+ (errrMsg.substring(0, errrMsg.length() - 1));
		}else {
			return "";
		}
