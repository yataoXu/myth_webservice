...
public class PageInterceptor implements Interceptor {  
       ...
       if(object instanceof Map){
    	   ...
    	   if(obj.containsKey("page")){
	    	   ...
	    	   if (null != page) {  
	    		   ...
	    		   //给当前的page参数对象设置总记录数  
	    		   this.setTotalRecord(page,object,mappedStatement, connection);  
	    		   //获取分页Sql语句  
     ...
     */  
    private void setTotalRecord(Page<?> page,Object obj,MappedStatement mappedStatement, Connection connection) {  
    ...
    private String getCountSql(String sql) {  
        if(sql.indexOf("order by")>=0){
        	int index1 = sql.indexOf("order by");
        	return "select count(*) from (" + sql.substring(0, index1) + ")";  
        }else{
